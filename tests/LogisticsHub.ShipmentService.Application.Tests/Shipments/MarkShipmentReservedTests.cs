...
public sealed class MarkShipmentReservedTests
{
    [Fact]
    public async Task Handle_WhenShipmentIsReservationRequested_MarksShipmentReserved()
    {
        // Arrange
        var dbContext = new FakeShipmentDbContext();
        var shipment = CreateShipment(ShipmentStatus.ReservationRequested);
        shipment.ReservationFailureReason = "Previous failure";
        dbContext.Shipments.Add(shipment);

        var reservationId = Guid.NewGuid();
        var command = new MarkShipmentReservedCommand(Guid.NewGuid(), shipment.Id, reservationId);
        ...
        Assert.Equal(reservationId, shipment.ReservationId);
